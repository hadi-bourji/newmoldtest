"""
Visualize model predictions side-by-side with ground truth labels.
Uses PyTorch (no TensorRT needed).

Usage:
    python visualize_predictions.py data/images/val --labels data/labels/val --output output_val
    python visualize_predictions.py data/images/val --labels data/labels/val --output output_val --checkpoint model_checkpoints/your_model.pth
"""
...
def match_preds_to_gt(pred_boxes, gt_boxes, iou_thresh=0.5):
    ...
    matched_gt = set()
    tp_counts = {}
    # Sort preds by score descending (higher confidence matched first)
...
def main():
    ...
    def onnx_infer(img_tensor):
        ...
        return torch.from_numpy(ort_out[0]).to(img_tensor.device)
    print(f"Using device: {device}")
    ...
    if os.path.exists(TRT_PATH):
        ...
        trt_context.set_tensor_address(trt_engine.get_tensor_name(0), trt_input_buf.data_ptr())
        trt_context.set_tensor_address(trt_engine.get_tensor_name(1), trt_output_buf.data_ptr())
        ...
        print(f"\n[TRT] Engine not found at {TRT_PATH}, skipping. Run python onnx_export.py to build it.")
